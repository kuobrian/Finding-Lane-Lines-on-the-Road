...
class Curves:
  ...
  def next_y(self, window):
    y_low = self.h - (window + 1) * self.window_height
    y_hight = self.h - window * self.window_height
    return y_low, y_hight
  ...
  def plot(self, t = 4):
    ...
    xls, xrs = None, None
    if self.left_pixels_indices.shape[0] != 0:
      ...
      xls = left_xs.astype(np.uint32)
      for xl, y in zip(xls, ys):
          cv2.line(self.out_img, (xl - t, y), (xl + t, y), (255, 255, 0), int(t / 2))
      
    
    if self.right_pixels_indices.shape[0] != 0:
  ...
  def fit(self, binary):
    ...
    print(self.left_pixels_indices.shape,  self.right_pixels_indices.shape)
